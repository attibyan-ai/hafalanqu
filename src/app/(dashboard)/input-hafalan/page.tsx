...
import { santriList, surahList } from "@/constants/mock-data";

const formSchema = z.object({
  santriId: z.string().min(1, "Pilih santri terlebih dahulu"),
...
}).refine(data => Number(data.ayatAkhir) >= Number(data.ayatMulai), {
              ...
              >
                <FormField label="Santri" required error={errors.santriId?.message}>
                  <Select value={watch("santriId")} onValueChange={(v) => setValue("santriId", v, { shouldValidate: true })}>
                    ...
                    <SelectContent>
                      {santriList.filter(s => s.status === "active").map((santri) => (
